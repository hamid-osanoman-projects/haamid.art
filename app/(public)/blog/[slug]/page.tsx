...
// Fallback mock posts mapped by slug
const MOCK_POSTS = [
  {
    id: 'p4',
...
`,
  ...
  },
  {
    ...
    content: `## What is the Movie Hacker Screen?
...
`,
  ...
  },
  {
    ...
    content: `## A Different Kind of Code Review
...
// Fetch helper matching DB or mock lists
async function getPostData(slug: string) {
  try {
    ...
    */
    ...
    console.warn('DB read skipped in post details lookup:', err);
  }
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const ogUrl = new URL('https://haaamid.art/api/og');
  ogUrl.searchParams.set('title', post.title);
  ogUrl.searchParams.set('category', post.category);

  return {
    title: post.title,
    description: post.excerpt || 'Hamid U V — web & software developer blog article.',
    openGraph: {
      type: 'article',
      title: post.title,
...
export default async function BlogPostPage({ params }: PageProps) {
  ...
  // View count increment mutation (updates database asynchronously if online)
  try {
    const supabase = createClient(
      process.env.NEXT_PUBLIC_SUPABASE_URL!,
      process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY!
    ...
    // Ignore increment failures during static page builds
  }

  // Get related reading suggestions (mock related cards logic)
  const related = MOCK_POSTS.filter(p => p.slug !== post.slug).slice(0, 3);

  // Schema for Article JSON-LD Structured Data
  const articleSchema = {
    "@context": "https://schema.org",
    "@type": "TechArticle",
    "headline": post.title,
    "description": post.excerpt,
    "image": post.cover_image || `https://haaamid.art/api/og?title=${encodeURIComponent(post.title)}&category=${post.category}`,
    ...
    "publisher": {
      ...
      "logo": {
        "@type": "ImageObject",
        "url": "https://haaamid.art/favicon.ico"
      }
    }
  };
